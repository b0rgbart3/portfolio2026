...
def extract_jd_signals_node(state: AssessState) -> dict:
    ...
    system_prompt = (
        ...
        "required programming languages, frameworks, tools, seniority level, "
        "domain (e.g. frontend, backend, ML), and key responsibilities. "
        "IMPORTANT: Only include years of experience if a specific number is explicitly stated in the job description. "
        "Do not infer, estimate, or add any requirements that are not explicitly written in the job description. "
...
def llm_assess_node(state: AssessState) -> dict:
    ...
    system_prompt = (
        ...
        "specific requirements EXPLICITLY stated in the JD that are clearly absent from Bart's documented profile. "
        "Do NOT list: (a) any requirement not explicitly written in the JD, "
        "(b) areas where Bart has equivalent or broader experience (e.g. full-stack covers frontend), "
        "(c) skills Bart clearly possesses even if the wording differs slightly. "
        "Only list genuine, evidence-based gaps. If there are no real gaps, return an empty list.\n"
